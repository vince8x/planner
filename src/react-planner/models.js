...
const safeLoadMapList = (mapList, Model, defaultMap) => {
  return mapList
...
export class Catalog extends Record({
  ...
  factoryElement(type, options, initialProperties) {
    ...
    const properties = element.properties.map((value, key) => initialProperties && initialProperties.has(key) ? initialProperties.get(key) : value.get('defaultValue'));
